...
import { Bookmark, Heart, Image as ImageIcon, Library, Sparkles, X, Share2, Copy, Check, Filter } from "lucide-react";
...
export function LibraryPage() {
  ...
  return (
    <AppShell>
        ...
        {(["Main Library", "My Library"] as Tab[]).map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative py-3.5 px-5 font-mono text-xs font-semibold cursor-pointer transition-colors ${
                isActive
                  ? "text-primary"
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto border-b border-border/50 px-4 py-2.5 scrollbar-none bg-background">
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`font-mono text-xs px-3 py-1 rounded-full transition-all cursor-pointer shrink-0 ${
            ...
            {displayedItems.map((item) => {
              ...
              return (
                <motion.div
                  ...
                  className="group relative cursor-pointer overflow-hidden rounded-xl border border-border/70 bg-card/40 transition-all hover:border-primary/60 hover:shadow-xl hover:-translate-y-0.5"
                >
                  {/* Image */}
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-between p-3">
                      ...
                      <div className="flex justify-end">
                        <button
                          ...
                          className={`rounded-full p-2 backdrop-blur-md transition-transform hover:scale-110 cursor-pointer ${
                            saved
      ...
      {/* ── IMAGE INTERACTION MODAL (Blurred backdrop + Split view) ── */}
      <AnimatePresence>
        {selectedItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="relative w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-2xl border border-border/80 bg-background shadow-2xl flex flex-col lg:flex-row"
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedItem(null)}
                className="absolute top-3 right-3 z-30 rounded-full bg-black/70 p-2 text-white hover:bg-black transition-colors cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 font-mono text-xs rounded-lg px-3.5 py-1.5 transition-all cursor-pointer ${
                  ...
                  <Link
                    ...
                    onClick={() => setSelectedItem(null)}
                    className="flex items-center gap-3 p-2.5 rounded-lg border border-border/60 bg-accent/20 hover:bg-accent/50 transition-colors mb-5"
                  >
                    <div
                  ...
                  <div className="mb-5 rounded-lg border border-border/80 bg-[#0a0a0c] p-3.5 shadow-inner">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 font-mono text-[11px] text-muted-foreground hover:text-primary transition-colors cursor-pointer bg-accent/40 px-2 py-0.5 rounded"
                      >
