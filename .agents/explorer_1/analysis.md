...
1. **TanStack Router Parent-Child Hierarchy vs Layout**:
   ...
   - _Impact_: Direct navigation to URL `/messages/c1` or `/profile/sora_dev` will attempt to render inside parent routes if hit directly. In current usage, `messages.tsx` uses internal React state `activeId`, and `profile.tsx` is used for `/profile` while `profile.$handle.tsx` handles `/profile/$handle`.
...
2. **Route Link Formats**:
   ...
   - _Recommendation_: Standardize all dynamic links to TanStack Router's typed `params` pattern.
